/**
 * A copy of `@stacksjs/browser`'s request-error module, for client code.
 *
 * It should be a package import, and stx 0.2.110 removed the bundler rule that
 * dropped `@stacksjs/browser` imports. Something upstream of the bundler still
 * strips them - most likely the auto-import transform, which rewrites browser
 * symbols to `window.StacksBrowser` lookups before the bundler ever sees the
 * import - so the page still ships calls to a function nothing defines.
 *
 * Until that is fixed the implementation lives here, where the bundler inlines
 * it. `@stacksjs/browser` holds the canonical version and its tests; keep the
 * two in step.
 */
