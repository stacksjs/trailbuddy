...
/**
 * Top-level framework-default models that belong to an optional feature
 * bundle. Unlike the gated SUBDIRS above (Content/, commerce/, …), these live
 * at the Models root, so without this gate they'd be auto-imported even when
 * the feature was never installed — reserving the name against app models and
 * declaring phantom FKs against tables the migration gate already suppresses
 * (e.g. CMS `comments`/`tags`). Mirror of the model entries in
 * `FEATURE_FILES` (core/buddy `commands/features.ts`) — kept local to avoid a
 * server→buddy import in the auto-import hot path; keep in sync if a bundle
 * adds a root model. Always-on infra (User, Notification, Log, …) is absent.
 */
const OPTIONAL_ROOT_MODELS: Record<string, string[]> = {
  // cms
  Comment: ['config/cms.ts', 'config/blog.ts'],
  Tag: ['config/cms.ts', 'config/blog.ts'],
  // marketing
  Campaign: ['config/marketing.ts'],
  CampaignSend: ['config/marketing.ts'],
  EmailList: ['config/marketing.ts'],
  EmailListSubscriber: ['config/marketing.ts'],
  SocialPost: ['config/marketing.ts'],
  // monitoring
  Error: ['config/monitoring.ts'],
  // queue
  Job: ['config/queue.ts'],
  FailedJob: ['config/queue.ts'],
}
...
/**
 * Whether a root-level framework-default model should be auto-imported: true
 * unless it's owned by an optional feature whose config is absent.
 */
function rootModelEnabled(basename: string): boolean {
  const gate = OPTIONAL_ROOT_MODELS[basename]
  return gate ? configEnabled(gate) : true
}
 ...
 */
// `gateRoot` marks the framework-defaults root scan: its top-level models are
// filtered through `rootModelEnabled` so bundle models stay out until installed.
type ScanEntry = string | { dir: string, recursive: boolean, gateRoot?: boolean }
...
async function generateDefineModelIndex(entries: ScanEntry[], outputPath: string): Promise<void> {
  ...
  for (const entry of entries) {
    ...
    const gateRoot = typeof entry === 'string' ? false : entry.gateRoot === true
    let files: string[] = []
    try {
      const pattern = recursive ? `${dir}/**/*.ts` : `${dir}/*.ts`
      files = globSync(pattern, { ignore: ['**/*.d.ts', '**/index.ts', '**/README*'] })
    ...
    for (const file of files) {
      ...
      if (!basename || seen.has(basename))
        continue
      // Skip framework-default models owned by a not-installed feature bundle.
      if (gateRoot && !rootModelEnabled(basename)) {
        lines.push(`// Skipped '${basename}' — owned by a disabled feature bundle. Run \`./buddy <feature>:install\` to enable.`)
        continue
      }
      seen.add(basename)
...
export async function generateAutoImportFiles(): Promise<void> {
  ...
  const modelScan: ScanEntry[] = [
    userModelsPath,
    { dir: defaultsRoot, recursive: false, gateRoot: true },
...
export function initiateImports(): void {
  ...
  // The defaults root is filtered so bundle models owned by a not-installed
  // feature aren't injected as globals (no name clash, no phantom-FK / global
  // shadowing like the monitoring `Error` model).
  const defineModelExports = [
    ...scanDefineModelExports(userModelsPath),
    ...scanDefineModelExports(defaultsRoot, { recursive: false }).filter(exp => rootModelEnabled(exp.name)),
